...
object GenericContainer {
  ...
  abstract class Def[C <: GenericContainer](init: => C) extends Container {
    override type Container = C
    protected def createContainer(): C = init
  }
}
abstract class GenericContainer(val underlyingUnsafeContainer: JavaGenericContainer[_]) extends ContainerRuntime {
