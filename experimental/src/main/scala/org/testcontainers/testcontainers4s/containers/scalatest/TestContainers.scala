...
import org.testcontainers.testcontainers4s.containers.{Container, ContainerList}
...
private[scalatest] object TestContainers {
  ...
  trait TestContainersSuite extends SuiteMixin { self: Suite =>
    ...
    @deprecated implicit def containerRuntime: Containers = startedContainers.get // TODO exception handling
...
trait TestContainerForEach extends TestContainersForEach { self: Suite =>
  ...
  final override type Containers = container.Container

  override def startContainers(): container.Container = {
    container.start()
  }
}
