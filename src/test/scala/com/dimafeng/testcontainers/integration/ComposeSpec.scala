...
class ComposeSpec extends FlatSpec with ForAllTestContainer {
  override val container = DockerComposeContainer(new File("src/test/resources/docker-compose.yml"), exposedServices = Map("redis_1" -> 6379))
...
class ComposeSpecWithImplicitConversions extends ComposeSpec {
  override val container = DockerComposeContainer(Seq(new File("src/test/resources/docker-compose.yml")), exposedServices = Map("redis_1" -> 6379))
}